...
noLayersCNN = 2
noFiltersCNN= 32 + 64 # will sum in loop 
hiddenActCNN= 'relu'
outputActCNN= 'relu'
dropOutsCNN = [0.0]
noLayersFNN = 2
noNeuronsFNN= 128 + 10 # willl sum in loop
hiddenActFNN= 'relu'
...
model.add(tf.keras.layers.Conv2D(filters=64, kernel_size=(3,3), activation=hiddenActCNN, padding='same')) 
model.add(tf.keras.layers.MaxPool2D(pool_size = (2,2)))
#model.add(tf.keras.layers.Dropout(0.5)) 

model.add(tf.keras.layers.Conv2D(filters=96, kernel_size=(3,3), activation=hiddenActCNN, padding='same'))
model.add(tf.keras.layers.MaxPool2D(pool_size = (2,2)))
#model.add(tf.keras.layers.Dropout(0.4))

model.add(tf.keras.layers.Conv2D(filters=128, kernel_size=(3,3), activation=hiddenActCNN, padding='same'))
model.add(tf.keras.layers.MaxPool2D(pool_size = (2,2)))
#model.add(tf.keras.layers.Dropout(0.4))
...
model.add(tf.keras.layers.Dense(128, activation=hiddenActFNN))    
model.add(tf.keras.layers.Dense(10, activation=outputActFNN))
...
#
#     20           21       22          23        24
# LEARNING RATE ; LOSS ; ACCURACY ; F1 SCORE ; PRECISION ; 
#
#   25        26           27 
...
contend += ";" + MyOptimizer
contend += ";" + MyLoss   
contend += ";" + str(0) #str(learningRate)
